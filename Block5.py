...
def phylogenetic_reduction(distance_matrix, labels):
    while len(distance_matrix) > 2:
        ...
        if v_j < 0:
            v_i = v_j + v_i  # Transfer the negative value to adjacent branch
    ...
    # Check if any branch length is negative, adjust accordingly
    if final_vi < 0:
        final_vj += final_vi  # Add the negative value to the other branch
        final_vi = 0  # Set the negative branch length to zero

    if final_vj < 0:
        final_vi += final_vj  # Add the negative value to the other branch
        final_vj = 0  # Set the negative branch length to zero
