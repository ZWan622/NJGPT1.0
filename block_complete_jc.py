...
def create_jc_distance_matrix(sequences):
    ...
    for i in range(num_seqs):
        for j in range(i + 1, num_seqs):
            distance = calculate_jc_distance(sequences[seq_names[i]], sequences[seq_names[j]])
...
# Calculate the distance matrix(JC model)
distance_matrix, seq_names = create_jc_distance_matrix(sequences)

print("Distance Matrix (JC):")
